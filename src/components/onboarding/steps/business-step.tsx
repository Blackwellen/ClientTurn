    ...
    <OPanel className="bg-[#0c151d]">
      <div className="flex items-start justify-between gap-2">
        <OInput
          ...
          className="h-8 border-none bg-transparent px-0 text-[14.5px] font-semibold focus:ring-0"
        />
        <div className="flex shrink-0 items-center gap-2 pt-0.5">
          ...
          <button
            ...
            onClick={onRemove}
            className="text-[#7a8698] transition-colors hover:text-[#ff6b70]"
          >
      ...
      <div className="mt-2 grid grid-cols-2 gap-2.5">
        <OField label="Average value" className="min-w-0">
          <OInput
            type="number"
            min={0}
        ...
        <OField label="Internal description (optional)" className="min-w-0">
          <OInput
                ...
                >
                  ...
                  <div className="mt-1.5 space-y-1">
                    <input
                      ...
                      value={entry.start}
                      onChange={(e) =>
                        setHours({ ...hours, [day.key]: { ...entry, start: e.target.value } })
                      }
                      aria-label={`${day.label} opens`}
                      className="h-7 w-full rounded-[5px] border border-[rgba(150,170,190,0.28)] bg-[#0d1720] px-1.5 text-[11.5px] text-[#dbe1ea] outline-none disabled:opacity-40"
                    />
                    <input
                      type="time"
                      disabled={!entry.open}
                      value={entry.end}
                      onChange={(e) =>
                        setHours({ ...hours, [day.key]: { ...entry, end: e.target.value } })
                      }

...
import { createSenderIdentityAction } from "@/lib/outreach/actions";

/**
 * The two entry points above the Campaigns list.
 *
 * They are owned by different decisions: the sending identity is "who this
 * business is when it writes to a stranger" and is set up once, while a
 * campaign is "what we are saying to whom" and there are many. So the identity
 * stays a dialog and the campaign is a six-step wizard on its own route.
 *
 * Neither creates something that sends. A campaign is created as a draft and
 * launched separately, so the moment a workspace starts emailing strangers is
 * always a distinct, deliberate act.
 */
...
export function CampaignBuilder({
  ...
  const blocked = !canManage || usable.length === 0;

  return (
    <div className="flex flex-wrap items-center gap-2">
      {/* A link, not a button: the wizard is a destination, so middle-click and
      ...
      >
        {blocked ? (
          ...
          <Link href="/app/find-leads/campaigns/new">
            <Plus className="size-4" aria-hidden />
            New campaign
          </Link>
        )}

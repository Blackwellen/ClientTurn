...
const OUTCOMES = [
  { title: "Qualified", colour: PALETTE.lime },
  { title: "Needs review", colour: "#E9B44C" },
  { title: "Not a fit", colour: "#D9636A" },
] as const;
...
export function SpeedToLead(props: ChapterProps) {
  ...
  return <>
    <Reveal {...props} position={[-2.5, 0, 0]}><Card title="New enquiry" subtitle="Meta lead form" height={1.1} active /></Reveal>
    ...
    <Reveal {...props} position={[1.6, -0.9, 0.3]} start={0.3}><Card title="Response sent" subtitle="Conversation started" height={1.1} width={2.6} active /><Status {...props} position={[1.1, -0.25, 0.16]} start={0.4} /></Reveal>
  </>;
}
...
export function FollowUp(props: ChapterProps) {
  ...
  return <>
    ...
    <Transfer {...props} path={[[3.5,0.9,-1.5],[2.6,0.1,0.6],[1.78,-0.55,1.2]]} start={0.45} end={0.78}><Card title="Reply received" subtitle="Follow-up stopped" height={1.05} width={2.05} active /></Transfer>
...
export function Qualification(props: ChapterProps) {
  return <>
    <Transfer {...props} path={[[-4.1,0.3,0],[-3.4,0,0.5],[-2.25,0,-0.62]]} start={0.08} end={0.55}><Card title="New enquiry" width={1.65} icon="profile" /></Transfer>
    <Shell width={2.3} height={2.8} depth={0.34} />
    {["Service Area", "Budget", "Timing", "Requirements"].map((title, i) => <Reveal key={title} {...props} position={[0, 0.95 - i * 0.63, 0.22]} start={0.1 + i * 0.12} end={0.25 + i * 0.12}><Card title={title} width={2.08} height={0.5} icon="none" /><Status {...props} position={[0.82, 0, 0.16]} start={0.1 + i * 0.12} /></Reveal>)}
    {OUTCOMES.map((outcome, i) => <group key={outcome.title}>
      <Branch points={[[1.14, (1 - i) * 0.1, -0.05], [1.5, (1 - i) * 0.5, -0.02], [1.78, (1 - i) * 0.9, i === 0 ? 0.14 : -0.42]]} active={i === 0} color={outcome.colour} dim={i !== 0} />
      <Reveal {...props} position={[2.62, (1 - i) * 0.9, i === 0 ? 0.2 : -0.62]} start={0.55} end={0.8}><Card title={outcome.title} width={1.62} height={0.66} icon="none" active={i === 0} color={outcome.colour} edge={outcome.colour} /></Reveal>
    </group>)}
  </>;
}
...
export function Booking(props: ChapterProps) {
  return <>
    <Card title="Qualified Lead" position={[-1.95, 0, 0]} width={1.7} height={1.6} icon="profile" active />
    <Calendar {...props} position={[0.6, 0.65, 0.15]} />
    <Reveal {...props} position={[2.55, 0.1, 0.05]} start={0.45} end={0.7}><Card title="Booking Confirmed" width={2.05} height={1.2} icon="none" active><Status {...props} position={[0, -0.23, 0.17]} start={0.5} /></Card></Reveal>
    <Branch points={[[-1.35, -0.4, 0], [-0.85, -0.85, -0.05], [-0.6, -1.7, 0.05], [-0.7, -1.95, 0.15]]} active={false} />
    <Card title="Needs Attention" position={[0.5, -1.78, 0.15]} width={2.4} icon="profile" />
    <Lettering text="HUMAN HANDOVER" position={[0.5, -2.42, 0.15]} width={2.4} height={0.2} color="#bfa468" />
    <Card title="Calendly" subtitle="Connected" position={[-2.85, -1.35, -0.3]} width={1.5} height={0.8} icon="none" />
    <Card title="Google Calendar" subtitle="Connected" position={[-2.85, -2.3, -0.3]} width={1.5} height={0.8} icon="none" />
  </>;
}
...
export function Reactivation(props: ChapterProps) {
  ...
  return <>
    ...
    <Reveal {...props} position={[2.18, -0.16, 0.35]} start={0.65} end={0.85}><Card title="Reply forwarded" subtitle="Back in conversation" width={1.72} height={1.1} active icon="none" /></Reveal>

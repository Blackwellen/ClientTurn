...
import { ACTIVE_INTENT_ONLY, applyProspectFilters } from "./filter-sql";
...
const LIST_SELECT = `
  ...
  campaign_id, source_provider, last_activity_at, created_at, promoted_to_lead_id,
  prospect_companies ( id, name, domain, website_url, industry, company_size,
...
export async function listProspects(
  ...
  return {
    rows: rows.map((row) => toListRow(row, intent, campaigns)),
    total: count ?? 0,
    page: filters.page,
